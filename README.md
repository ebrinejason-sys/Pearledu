...
**Fees / SchoolPay:** Parent portal can initiate SchoolPay MoMo debit requests when a school enables SchoolPay under School identity. Channel payments (student payment codes) arrive via webhook or `php artisan schoolpay:sync`. Manual cash/MoMo “submit for verification” remains available as a fallback.
