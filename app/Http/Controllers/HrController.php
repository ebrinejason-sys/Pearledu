...
class HrController extends Controller
{
    public function index(TenantContext $ctx)
    {
        ...
        $user = request()->user();
        $canManageHr = $user && in_array('hr.manage', $user->permissionsForSchool($school->id), true);
        $leaves = LeaveRequest::where('school_id', $school->id)->with('user')->orderByDesc('id')->get();

        return view('app.hr.index', compact('school', 'leaves', 'canManageHr'));
    }
    ...
    {
        $school = $ctx->school();
        abort_unless($school && (int) $leave->school_id === (int) $school->id, 404);
