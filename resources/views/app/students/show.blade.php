...
@php
  $tab = $profileTab ?? 'basic';
  $tabUrl = fn (string $name) => route('app.students.show', ['student' => $student, 'tab' => $name]);
@endphp
@section('content')
  <div class="page-header" style="display:flex;justify-content:space-between;align-items:flex-end;gap:16px;flex-wrap:wrap;margin-bottom:16px">
    <div>
      <p class="page-header__eyebrow">Learner Profile</p>
      <h1 class="page-header__title" style="margin:0">{{ $student->full_name }}</h1>
      <p style="color:var(--muted);margin:6px 0 0">
        {{ $student->full_name }}
        @if($student->emis_number)
          ({{ $student->emis_number }})
        @endif
        Details
      </p>
  ...
  <div class="lp">
    <aside class="lp-rail">
      <div class="lp-rail__who">
        @if($student->photoUrl())
          <img src="{{ $student->photoUrl() }}" alt="" width="96" height="96" class="lp-rail__photo">
        @else
          <span class="learner-avatar learner-avatar--empty lp-rail__photo" aria-hidden="true"></span>
        @endif
        <div class="lp-rail__name">{{ $student->full_name }}</div>
        <div class="lp-rail__meta">
          Age {{ $student->ageYears() !== null ? $student->ageYears().' yrs' : '—' }}
          · Sex {{ strtoupper(\App\Support\Gender::label($student->gender)) }}
        </div>
        <span class="pill">{{ $student->status }}</span>
      </div>
      <nav aria-label="Learner sections">
        <ul class="lp-nav">
          <li><a href="{{ $tabUrl('basic') }}" class="{{ $tab === 'basic' ? 'is-active' : '' }}">Basic Info</a></li>
          <li><a href="{{ $tabUrl('guardians') }}" class="{{ $tab === 'guardians' ? 'is-active' : '' }}">Parents/Guardian</a></li>
          @if(!empty($canViewFinance) || !empty($canApplyFees))
            <li><a href="{{ $tabUrl('fees') }}" class="{{ $tab === 'fees' ? 'is-active' : '' }}">Fees</a></li>
          @endif
          <li><a href="{{ $tabUrl('login') }}" class="{{ $tab === 'login' ? 'is-active' : '' }}">Login</a></li>
        </ul>
      </nav>
    </aside>
    ...
    <div class="lp-panel">
      @if($tab === 'basic')
        <div class="lp-tabs">
          <span class="is-active">PearlEdu details</span>
        </div>
        <h3 style="margin-top:0">Personal information</h3>
        <dl class="lp-dl">
          <dt>Full name</dt><dd>{{ $student->full_name }}</dd>
          <dt>Date of birth</dt><dd>{{ $student->date_of_birth ? strtoupper($student->date_of_birth->format('d F, Y')) : '—' }}</dd>
          <dt>Nationality</dt><dd>{{ $student->nationality ?: 'Uganda' }}</dd>
          <dt>Class / stream</dt><dd>{{ $student->schoolClass?->displayName() ?: '—' }}</dd>
          <dt>Day / boarding</dt><dd>{{ \App\Support\Residency::label($student->residency) }}</dd>
          <dt>EMIS number</dt><dd>{{ $student->emis_number ?: '—' }}</dd>
          <dt>LIN on file</dt><dd>{{ $student->hasLinOnFile() ? 'Yes' : 'Not on file' }}</dd>
          <dt>NIN on file</dt><dd>{{ $student->hasNinOnFile() ? 'Yes' : 'Not on file' }}</dd>
          <dt>Religion</dt><dd>{{ $student->religion ?: '—' }}</dd>
          <dt>Home address</dt><dd>{{ $student->home_address ?: '—' }}</dd>
          <dt>Medical notes</dt><dd>{{ $student->medical_notes ?: '—' }}</dd>
      ...
      @if($tab === 'guardians')
        <h3 style="margin-top:0">Parents / Guardian</h3>
      ...
      @if($tab === 'fees' && (!empty($canViewFinance) || !empty($canApplyFees)))
        ...
        @include('app.students._fee_attach', [
          'layout' => 'profile',
          'student' => $student,
          'canApplyFees' => $canApplyFees ?? false,
