...
- **Demo data:** `php artisan school:seed-walkthrough --password='<pw>'` seeds St. Kizito Demonstration Primary (`pearledu1`) with accounts like `admin@stkizito.test`, `english@stkizito.test`, `bursar@stkizito.test`, `secretary@stkizito.test` (see README table). Invited staff stay `status=invited` and cannot sign in until they open the mail link or you run `php artisan invite:activate email@school.test --password='…'` (add `--force` on a live server). New catalog roles (e.g. secretary) need `php artisan db:seed --class=RoleSeeder` after deploy — config alone does not insert the `roles` row.
