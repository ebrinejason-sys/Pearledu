...
- `ResolveTenant` IS applied globally to `/login` (prepended to the `web` middleware group in `bootstrap/app.php`, not route-scoped) — it resolves the active school (and therefore theme) from the request's Host header. A plain `$this->get('/login')` (no host prefix) hits an unrecognized host and fail-closes to the default theme, which is sufficient for the layout/markup/mobile tests below, but the theme-token requirement must additionally be verified against a real tenant subdomain (`http://{school-slug}.voxsign.test/login`, per `TenantIsolationTest`'s pattern of `TenantContext::forPlatform()` + `School::create(...)`) to prove per-school theming actually activates, not just that *some* theme's tokens render.
